...
extension UIView {
    ...
    func shake(_ offsetX:CGFloat, offsetY:CGFloat,duration:CFTimeInterval? = 0.06, repeatCount:CGFloat? = 3) {
    ...
    func scale(_ by:CGFloat, duration:CFTimeInterval? = 0.1) {
    ...
    func rotate(_ rotate:CGFloat, duration:CFTimeInterval? = 2.0) {
        let animation = CABasicAnimation(keyPath: key_transform)
        animation.duration = duration!
        animation.isRemovedOnCompletion = false
        animation.fillMode = kCAFillModeForwards
        animation.toValue = NSValue.init(caTransform3D: CATransform3DMakeRotation(CGFloat(M_PI)*rotate, 0.0, 0.0, 1.0))
        self.layer.add(animation, forKey: nil)
    }
