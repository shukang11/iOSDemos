...
extension String {
    ...
    /// - Returns: 返回拼接后的字符串
    func stringByAppendingNameScale(_ scale:CGFloat) -> String {
    ...
    ///   - mode: 换行类型
    /// - Returns: 返回的字符串占用宽高
    func getSize(togolFont font:UIFont?=UIFont.systemFont(ofSize: 12.0), togolSize size:CGSize, breakMode mode:NSLineBreakMode) -> CGSize {
    ...
    /// - Parameter font: 字体字号
    /// - Returns: 占用宽度
    func getWidth(togolFont font: UIFont) -> CGFloat {
    ...
    ///   - width: 限制宽度
    /// - Returns: 占用的高度
    func getHeight(togolFont font: UIFont, _ width:CGFloat) -> CGFloat {
